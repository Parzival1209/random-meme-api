...
The parameter count controls the number of returned URLs. The minimum value for count is 1, and the maximum value is the size of the currently selected meme database (See height and width for more info). *Note that this parameter only works with JSON and plain text formats.* <br>
